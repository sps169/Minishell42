...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/21 10:56:18 by migonzal          #+#    #+#             */
/*   Updated: 2023/03/15 10:49:16 by migonzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	redir_out_error(t_copy *copy, int i)
{
	...
	copy->redir.out2 = malloc(sizeof(char) * ft_strlen(copy->wc) + 1);
	if (!(copy->redir.out2))
		return (-1);
	while (copy->wc[copy->i] && copy->wc[copy->i] == ' ')
		copy->i++;
