...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/21 10:39:48 by migonzal          #+#    #+#             */
/*   Updated: 2023/03/15 10:49:13 by migonzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_file_out2(t_copy *copy)
{
	...
	if (copy->redir.end == 1)
		copy->redir.sstderr = open(copy->redir.out2, O_CREAT
...
int	create_file(t_copy *copy, int type)
{
	...
	if (type == 1)
	{
		...
		if (copy->redir.end == 1)
			copy->redir.sstdout = open(copy->redir.out1, O_CREAT
					| O_RDWR | O_APPEND, 0644);
		else
			copy->redir.sstdout = open(copy->redir.out1, O_CREAT
					| O_RDWR | O_TRUNC, 0644);
...
int	redir_in(t_copy *copy, int i)
{
	...
	copy->redir.in = malloc(sizeof(char) * ft_strlen(copy->wc) + 1);
	if (!(copy->redir.in))
		return (-1);
