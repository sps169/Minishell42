...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/22 11:11:00 by migonzal          #+#    #+#             */
/*   Updated: 2023/03/15 10:49:06 by migonzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (multiple_dollars(copy, arg, i) != 0 || status_env(copy, arg, i) == 1)
		return (1);
	...
	if (space == 1 && copy->redir.value)
		copy->redir.value = ft_strip_extra_spaces(copy->redir.value,
				copy->wc, copy->i);
	space = no_value(copy, copy->redir.value);
	if (space != 0)
	{
		if (copy->redir.value)
...
void	env_redir_ambig(t_copy *copy, int space)
{
	...
	if (space == 1 && copy->redir.value)
		copy->redir.value = ft_strip_extra_spaces(copy->redir.value,
				copy->wc, copy->i);
	if (copy->redir.value && space == 1 && copy->wc[copy->i] != '$'
		&& (ft_only_spaces(copy->redir.value)
...
int	env_redir(t_copy *copy, int std, int spce)
{
	...
	count = no_value(copy, copy->redir.value);
	if (count != 0)
	{
		if (copy->redir.value)
			free(copy->redir.value);
